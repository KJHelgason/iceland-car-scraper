#!/usr/bin/env python3
"""Check Bilaland listings without images to see if they're inactive."""
...
async def check_inactive_bilaland_listings():
    """Check each Bilaland listing without an image to see if it's inactive."""
    
    session = SessionLocal()
    
    # Get all active Bilaland listings without images
    # Only check listings that are currently marked as active
    listings_no_img = session.query(CarListing).filter_by(source='Bilaland', is_active=True).filter(
        (CarListing.image_url == None) | (CarListing.image_url == '')
    ).all()
    
    print(f"Found {len(listings_no_img)} active Bilaland listings without images")
    ...
    async with async_playwright() as p:
        ...
        for idx, listing in enumerate(listings_no_img, 1):
            try:
                ...
                try:
                    inactive_message = await page.query_selector('xpath=/html/body/form/div[3]/div[3]/div/div/div')
                    if inactive_message:
                ...
                try:
                    img = await page.query_selector('xpath=/html/body/form/div[3]/div[3]/div/div/div/div[1]/a/img')
                    if img:
                        image_url = await img.get_attribute('src')
                        if image_url:
                            if not image_url.startswith('http'):
                                image_url = f"https://www.bilaland.is{image_url}"
    ...
    print(f"\n=== Check Complete ===")
    print(f"Inactive listings: {inactive_count}")
    ...
    if inactive_ids:
        ...
        print(f"Found {len(inactive_ids)} inactive listings")
        print("These could be deleted from the database:")
        for listing_id in inactive_ids[:20]:  # Show first 20
            inactive = session.query(CarListing).get(listing_id)
            if inactive:
        ...
        print(f"\nTo delete these inactive listings, you can run:")
        print(f"DELETE FROM car_listings WHERE id IN ({','.join(map(str, inactive_ids[:100]))});")
    
    session.close()


if __name__ == "__main__":
    asyncio.run(check_inactive_bilaland_listings())
